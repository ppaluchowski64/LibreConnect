...
site_dir="${2:-site}"
repo_dir="$site_dir/packages/pacman"
...
for arch in x86_64 aarch64; do
    ...
    for package_file in "$arch_dir"/*.pkg.tar.zst; do
        sign_detached_ascii "$package_file" "$package_file.sig"
    done

    repo-add "$arch_dir/$repo_name.db.tar.gz" "$arch_dir"/*.pkg.tar.zst
    sign_detached_ascii "$arch_dir/$repo_name.db.tar.gz" "$arch_dir/$repo_name.db.tar.gz.sig"
    cp -a "$arch_dir/$repo_name.db.tar.gz.sig" "$arch_dir/$repo_name.db.sig"
    
    if [[ -f "$arch_dir/$repo_name.files.tar.gz" ]]; then
        sign_detached_ascii "$arch_dir/$repo_name.files.tar.gz" "$arch_dir/$repo_name.files.tar.gz.sig"
...
Server = $REPO_URL/packages/pacman/$arch
EOF_CONF
done
