...
[libreconnect]
name=LibreConnect
baseurl=$REPO_URL/dnf/$arch
enabled=1
type=rpm-md
gpgcheck=1
repo_gpgcheck=1
gpgkey=$REPO_URL/dnf/RPM-GPG-KEY-libreconnect
metadata_expire=6h
skip_if_unavailable=False
EOF_REPO
done

cat > "$repo_dir/libreconnect.repo" <<EOF_REPO
[libreconnect]
name=LibreConnect
baseurl=$REPO_URL/dnf/\$basearch
enabled=1
type=rpm-md
gpgcheck=1
repo_gpgcheck=1
