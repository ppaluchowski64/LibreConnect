...
asio::awaitable<void> Execute() {
    ...
    asio::co_spawn(context, serverChannel->SendFile(std::filesystem::path("./test.txt")), asio::detached);
    auto fut = asio::co_spawn(context, clientChannel->ReceiveFile(std::filesystem::path("./result.txt")), asio::use_future);
