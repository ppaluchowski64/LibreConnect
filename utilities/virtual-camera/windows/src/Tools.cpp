...
const std::wstring GetProcessName(DWORD pid)
{
	if (pid)
	{
		...
		if (handle)
		{
			...
			std::wstring ws;
			ws.resize(size);
			QueryFullProcessImageNameW(handle, 0, ws.data(), &size);
			CloseHandle(handle);
...
{
	*outKey = nullptr;
	return RegCreateKeyExW(key, path, 0, nullptr, 0, KEY_WRITE, nullptr, outKey, nullptr);
}

const LSTATUS RegWriteValue(HKEY key, PCWSTR name, const std::wstring& value)
{
	return RegSetValueExW(key, name, 0, REG_SZ, reinterpret_cast<BYTE const*>(value.c_str()), static_cast<uint32_t>((value.size() + 1) * sizeof(wchar_t)));
}

const LSTATUS RegWriteValue(HKEY key, PCWSTR name, DWORD value)
{
	return RegSetValueExW(key, name, 0, REG_DWORD, reinterpret_cast<BYTE const*>(&value), sizeof(value));
}
