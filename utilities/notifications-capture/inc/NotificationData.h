...
public:
    NotificationData();
    NotificationData(std::string key, std::string title, std::string content, size_t timestamp, std::optional<std::vector<uint8_t>> icon);

    std::string key;
    std::string title;
    std::string content;
    size_t timestamp{0};
    std::optional<std::vector<uint8_t>> icon;
